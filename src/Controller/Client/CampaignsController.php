...
class CampaignsController extends AppController
{
    ...
    {
        $this->paginate = [
            // 'contain' => ['Users', 'Calls', 'Gateway'],
            'contain' => ['Clients', 'Gateway'],
        ...
        $campaigns = $this->Campaigns->find('all')->where([
            'Campaigns.client_id' => $this->request->getSession()->read('client_id')
        ]);//->contain(['Clients', 'Gateway'])->orderDesc('Campaigns.id');


        $campaigns = $this->paginate($campaigns);
    
        foreach($campaigns as $key => $campaign){
            $campaign->total_action =  $Actions->find('all')->where(['Actions.campaigns_id' => $campaign->id])->count();
            $campaign->total_contact = $Contacts->find('all')->where(['Contacts.contact_groups_id IN ' => explode(',', $campaign->contact_groups)])->distinct(['Contacts.mobile_no'])->count();
        }

        $audios = $Audios->find('all')->where([
            'Audios.client_id' => $this->request->getSession()->read('client_id')
        ])->all();


        $this->set('status', $this->status);
        $this->set(compact('campaigns', 'audios'));
    }
    ...
    public function view($id = null)
    {
        $campaign = $this->Campaigns->get($id, [
            'contain' => ['Clients', 'Calls', 'Gateways'],
        ]);
    ...
    public function change($id, $status){
        ...
        $campaign  =  $this->Campaigns->find('all')->where(["Campaigns.id" => $id, "client_id" => $client_id ])->first();
        ...
        if($campaign) {
            ...
            if($this->Campaigns->save($campaign))
                ...
                $this->Flash->error("Status could not be Change. please try again.");
        }else{
            $this->Flash->error("Campaign Could not found. plase try again");
        }
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            $data['contact_groups'] = implode(',', $data['contact_groups']);
            $wlist = array();
            foreach( $data['weekdays'] as $key => $value){
                if($value == 1) array_push($wlist, $key);
            }
            $data['weekdays'] = implode(',', $wlist);
            $data['client_id']  = $client_id;
            if($data['max_call_retry'] == 0)
                $data['max_call_retry'] = 1;

            $campaign = $this->Campaigns->patchEntity($campaign, $data);
            if ($this->Campaigns->save($campaign)) {
                $this->Flash->success(__('The campaign has been saved.'));
                if( isset($data['setup_ivr']) && $data['setup_ivr'] == 'on'){
                    return $this->redirect(['controller' => 'actions', 'action' => 'add', $campaign->id]);    
                }
                // pr($data);
                // die();
                return $this->redirect(['action' => 'index']);
        ...
        $ContactGroups = TableRegistry::getTableLocator()->get('ContactGroups');
        $contactGroups = $ContactGroups->find('list',
            [
                'keyField' => 'id',
                'valueField' => 'group_name'
            ])->where(['ContactGroups.client_id' => $client_id ]);

        $gateways = $this->Campaigns->Gateway->find('list',
            [
                'keyField' => 'id',
                'valueField' => 'name'
            ])->where(['Gateway.client_id' => $client_id ]);//->orWhere(['Gateway.created_by_admin' => 1]);
        
        $audios = $this->Campaigns->Audios->find('list',
            [
                'keyField' => 'id',
                'valueField' => 'caption'
            ])->where(['Audios.client_id' => $client_id ]);

        $_audios = $this->Campaigns->Audios->find('all')->where([
                'Audios.client_id' => $this->request->getSession()->read('client_id')
            ])->all();
        
        $this->set('timezones', $this->timezoneList());
        $this->set(compact('campaign', 'contactGroups', 'gateways', 'audios', '_audios'));
    ...
    {
        $client_id = $this->request->getSession()->read('client_id'); 
        $campaign = $this->Campaigns->find('all')->where([
            'Campaigns.id' => $id,
        ...
        if(!$campaign){
            $this->Flash->error('You have no permission to update others campaign');
            return $this->redirect(['action' => 'index']);
        }

        if($campaign->status == 5 || $campaign->status == 6){
            $this->Flash->error("You can not edit campaign if this campaign has already in {$this->status[$campaign->status]} status");
            return $this->redirect(['action' => 'index']);
        }
                
        if ($this->request->is(['patch', 'post', 'put'])) {
            $data = $this->request->getData();
            if(!isset($data['connect_pbx'])){
                $data['pbx_number'] = null;
                $data['pbx_gateway_id'] = null;
            }
            else
                $data['audios_id'] = null;
            ...
            if($data['max_call_retry'] == 0)
                $data['max_call_retry'] = 1;
            $data['contact_groups'] =  empty($data['contact_groups']) ? $campaign->contact_groups : implode(',', $data['contact_groups']);
        ...
        $campaign['contact_groups'] = explode(',', $campaign['contact_groups']);
        $this->set('timezones', $this->timezoneList());
        $this->set(compact('campaign', 'contactGroups', 'gateways', 'audios', '_audios'));
    }

    public function readyCam($id = null){
        ...
        $contacts = $Contacts->find('all')->where(['Contacts.contact_groups_id IN ' => explode(',', $campaign->contact_groups)])->distinct(['Contacts.mobile_no'])->all();
        foreach($contacts as $contact){

            $call = $Calls->find('all')->where([
                'Calls.contact_number'  => $contact->mobile_no,
            ...
            if(!$call){
                $data = [
                    ...
                    'contact_number'    => $contact->mobile_no,
                    // 'retry_count'       => 0,
                    // 'call_duration'     => 0,
                    'caller_status'     => 0
                ];
                ...
                $call = $Calls->patchEntity($call, $data);
        ...
        if($success) {
            ...
            if($this->Campaigns->save($campaign))
                $this->Flash->success("Contact transfer to Call list successfully.Success #{$success}, Failed #{$fail}, Duplicate #{$duplicate} ");
            else
                $this->Flash->error("Contact transfer to Call list successfully, but campaign statu could not be updated");    
    ...
    public function report($campaign_id = null){
        $Calls      = TableRegistry::getTableLocator()->get('Calls');
        $Contacts   = TableRegistry::getTableLocator()->get('Contacts');
        $campaign   = $this->Campaigns->find('all')->where(['id' => $campaign_id])->first(); 
        
        $this->paginate = array(
            'limit' => 50,
            'order' => ['Calls.connect_time' => 'desc']
        );
        
        $calls = $Calls->find('all')->where([
            'Calls.campaign_id' => $campaign_id,
            'Calls.client_id'   => $this->request->getSession()->read('client_id')
        ])->contain(['Campaigns', 'Contacts']);
        ...
        if(!$calls)
            ...
            if ( isset($data['contact_number']) && !empty($data['contact_number']))
                $calls->where([ 'Calls.contact_number' => trim( $data['contact_number'] ) ]);
            
            if(isset($data['call_status']) && !empty($data['call_status'])){
                if($data['call_status'] == 7){
                    $calls->where(['Calls.call_status IS ' => NULL]);
                }

                else if($data['call_status'] == 6){
                    $calls->where(['Calls.call_status' => 0, 'Calls.connect_time !=' => 0 ]);
                }

                else if($data['call_status'] == 5){
                    $calls->where(['Calls.call_status' => 4, 'Calls.call_duration' => 0 ]);
                }
                
                elseif($data['call_status'] == 4){
                    $calls->where([
                        'Calls.call_status' => 4,
                    ...
                    $calls->where([ 'Calls.call_status' => $data['call_status'] ]); // 3-connecting, 2-ringing, 1-trying
            }

            if(isset($data['download']) && $data['download'] == 1 && $calls->count() > 0){
                if(isset($data['call_status']) && !empty($data['call_status']))
                    $filter_keyword = $this->call_status[$data['call_status']];

                elseif(isset($data['contact_number']) && !empty($data['contact_number']))
                    $filter_keyword = $data['contact_number'];

                $filename = str_replace(' ', '-', $campaign->name) . "_" . $filter_keyword . ".csv";
                header("Content-type: text/csv");
                header("Content-Disposition: attachment; filename={$filename}");
                $output = fopen('php://output', 'w');
                fputcsv($output, ['mobile_no', 'contact_name', 'address']);
                foreach ($calls->contain(['Contacts']) as $call) {
                    $data= [
                        $call->contact->mobile_no, 
            ...
            if(isset($data['download']) && $data['download'] == 1 && $calls->count() == 0)
                $this->Flash->error("Did not find any contact to export");
            

        $this->set('status', $this->status);
        $this->set('calls', $this->paginate($calls));
        $this->set('campaign', $campaign);
        $this->set('camid', $campaign_id);
        

    }


    public function printHistory($campaign_id){
        $this->layout = "ajax";
        if(!$campaign_id) return $this->redirect(['action' => 'index']);
        
        $client = $this->request->getSession()->read('client'); 
        $campaign = $this->Campaigns->find('all')->where([
            'Campaigns.id' => $campaign_id,
            // 'Campaigns.status' => 6,
            'Campaigns.client_id' => $client->id
        ])->contain(['Clients', 'Gateway'])->first();

        if(!$campaign){
            $this->Flash->error("You have no permission to print others campaign OR if it is not completed");
            return $this->redirect(['action' => 'index']);
        }
        // pr($campaign);
        // die();
        ...
        $first_call = $Calls->find('all')->where(['Calls.campaign_id' => $campaign_id])->orderAsc('Calls.connect_time')->first();
        $last_call  = $Calls->find('all')->where(['Calls.campaign_id' => $campaign_id])->orderDesc('Calls.disconnect_time')->first();
        
        if($first_call)
            $campaign->started = $first_call->connect_time != 0 ? date("d-M-y h:i A", $first_call->connect_time) : "00-00-00 00:00";
        
        if($last_call)
            $campaign->finished = $last_call->disconnect_time != 0 ? date("d-M-y h:i A", $last_call->disconnect_time) : "00-00-00 00:00";

        $this->set('call_status', $this->call_status);

        $this->set(compact('campaign', 'calls', 'client'));

    }

    public function printInfo($campaign_id){
        $this->layout = "ajax";
        if(!$campaign_id) return $this->redirect(['action' => 'index']);
        ...
        $total_send_sms = 0;
        $sms = $Calls->find('all')->where([
            'Calls.campaign_id' => $campaign_id,
            // 'Calls.event_received'    => 1,
            // 'Calls.call_action_type'  => 1,
            'Calls.call_action_status >'=> 0
        ])->all();

        foreach($sms as $sm){
            $total_send_sms += $sm->call_action_status;
        }
        ...
        $calls = $Calls->find('all')->where(['Calls.campaign_id' => $campaign_id]);

        
        //$data =["Received" => 0, "Failed" => 0, "Connected" => 0, "Ringing" => 0, "Trying" => 0, "Connecting" => 0];
        $data =["Received" => 0, "Failed" => 0];

        foreach( $calls as $key => $call){
            if($campaign->gateway->created_by_admin){
                $call_pulse = isset($campaign->gateway->call_pulse) && $campaign->gateway->call_pulse > 0  ? $campaign->gateway->call_pulse : 30;
                $reminder = $call->call_duration % $call_pulse;
                $report['call_duration'] += $reminder > 0 ?  ($call->call_duration - $reminder) + $call_pulse : $call->call_duration;
            }
            else
                $report['call_duration'] += $call->call_duration;
            ...
            /*
            else if ($call->call_status == 4 && $call->call_duration == 0 && $call->retry_count == $campaign->max_call_retry)
                $data['Failed'] += 1;

            else if ($call->call_status == 4 && $call->call_duration == 0)
                $data['Failed'] += 1;   
            
            else if ($call->call_status == 3)
                $data['Connected'] += 1;
            
            else if ($call->call_status == 2)
                $data['Ringing'] += 1;
            else if ($call->call_status == 1)
                $data['Trying'] += 1;
            else if ($call->call_status == 0)
                $data['Connecting'] += 1;
            */
        }
        $report["total_call"] = $calls->count();
   
        $first_call = $Calls->find('all')->where(['Calls.campaign_id' => $campaign_id])->orderAsc('Calls.connect_time')->first();
        $last_call  = $Calls->find('all')->where(['Calls.campaign_id' => $campaign_id])->orderDesc('Calls.disconnect_time')->first();
    
        if($first_call)
            $report['first_call_start'] = $first_call->connect_time != 0 ? date("d-M-y h:i A", $first_call->connect_time) : "00-00-00 00:00";
        
        if($last_call)
            $report['last_call_end'] = $last_call->disconnect_time != 0 ? date("d-M-y h:i A", $last_call->disconnect_time) : "00-00-00 00:00";
        
        $report['call_duration_in_hours'] = $this->secToHR($report['call_duration']);


        $CallForwarding = TableRegistry::getTableLocator()->get('CallForwarding');
        $forward_call_duration = 0;
        $call_forward_gw_created_by_admin = false;

        $forward_calls = $CallForwarding->find('all')->where([
            'CallForwarding.campaigns_id' => $campaign->id
        ])->contain(['Gateway'])->all();
        
        foreach($forward_calls as $forward_call){
            if($forward_call->gateway->created_by_admin && $forward_call->call_duration > 0){
                $call_forward_gw_created_by_admin = true;
                if(isset($forward_call->gateway->call_pulse) && $forward_call->gateway->call_pulse > 0){
                    $call_pulse = $forward_call->gateway->call_pulse;
                    $reminder = $forward_call->call_duration % $call_pulse;
                    $forward_call_duration += $reminder > 0 ?  ($forward_call->call_duration - $reminder) + $call_pulse : $forward_call->call_duration;
                }
                else{
                    $forward_call_duration += $forward_call->call_duration;
                }
            }
        }

        $report['forward_call_duration_in_hours'] = $this->secToHR($forward_call_duration);

        if($forward_calls->count() > 0){
            $gateway = $forward_calls->first()->gateway;
            if($gateway->created_by_admin){
                $forward_call_cost = ($forward_call_duration / 60) * $gateway->call_rate;
                $report['forward_call_cost'] = number_format(($forward_call_cost / 100), 2);
            }
        }

        $client = $this->request->getSession()->read('client');
        $this->set(compact('report', 'calls', 'campaign', 'data', 'total_send_sms', 'client', 'call_forward_gw_created_by_admin'));
    }
    ...
    public function delete($id = null)
    {
        ...
        if ($campaign->status == 0) {
            ...
            $this->Flash->error(__('The campaign could not be deleted.'));
        }
    ...
    public function secToHR($seconds) {
        ...
        $hours = floor((int) $seconds / 3600)." Hour ";

        if($hours>1)
            $hours = floor( (int) $seconds / 3600)." Hours ";
        elseif ($hours==0)
            $hours=null;
        
        $minutes = floor(( (int) $seconds / 60) % 60)." Minute ";
        if($minutes>1)
            $minutes = floor(( (int) $seconds / 60) % 60)." Minutes ";
        elseif ($minutes==0)
            $minutes=null;
        
        $seconds =( (int) $seconds % 60)." Second";
